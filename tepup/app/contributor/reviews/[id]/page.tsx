...
export default function ReviewDetailPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      router.push('/contributor/reviews');
      router.refresh();
    } catch {
  ...
  if (error && !contribution) {
    return (
      <div className="p-12 text-center">
        ...
        <Link href="/contributor/reviews" className="text-teal-600 text-sm mt-2 inline-block">
          Quay lại
        </Link>
      </div>
    );
          ...
          {isPending && (
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              ...
              <button
                ...
                className="w-full py-2.5 bg-teal-500 text-white rounded-lg font-medium text-sm hover:bg-teal-600 disabled:opacity-50 transition-colors"
              >
